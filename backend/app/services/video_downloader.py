...
def _format_fallback_options(base_options: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    broader_format_options = base_options.copy()
    broader_format_options["format"] = "bestvideo*+bestaudio/best"
    fallback_options.append(broader_format_options)
    ...
    fallback_options.append(best_single_stream_options)
    ...
    fallback_options.append(default_selector_options)
    ...
    *,
    ...
    try:
        ...
        if _is_format_unavailable_error(exc):
            last_fallback_error: DownloadError | None = None
            ...
            if last_fallback_error and _is_format_unavailable_error(last_fallback_error):
                raise VideoDownloadError(
                    "Failed to download the provided URL. No compatible video format was available."
                ) from last_fallback_error

            raise VideoDownloadError("Failed to download the provided URL. Check the link and try again.") from (
                last_fallback_error or exc
            )
